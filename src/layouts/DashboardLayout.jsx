...
import {
  ...
  Server, Briefcase, Sun, Moon, Laptop
} from 'lucide-react';
...
// Components
import Restricted from '../components/Restricted'; // <--- Import Restricted Component
...
const DashboardLayout = () => {
  ...
  // --- ROLE CHECK (Kept for internal logic if needed, but UI uses Restricted) ---
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') { e.preventDefault(); openModal('commandPalette'); return; }
      ...
      if ((e.key === '?' || e.key === '/') && !isTyping) { e.preventDefault(); if (!Object.values(modalState).some(isOpen => isOpen)) { openModal('shortcuts'); } return; }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [openModal, modalState, canUseAI]);

  useEffect(() => {
    if (isFirstLogin) {
      const timer = setTimeout(() => { setShowOnboardingModal(true); }, 1500);
  ...
  const getPageTitle = () => {
    ...
    if (path === '/pricing') return 'Plans & Billing';
    return 'Dashboard';
  };

  const getPageDescription = () => {
    return 'Manage your team efficiently.';
  };

  const handleNavigate = () => { setIsMobileSidebarOpen(false); };
  ...
  const renderSidebarLinks = (isCollapsed) => (
    <>
      ...
      <Restricted to="canUseAI">
        <button onClick={() => { openModal('aiChat'); handleNavigate(); }} className={`group flex items-center cursor-pointer ${isCollapsed ? 'justify-center' : 'space-x-3'} px-3 py-3 rounded-xl transition-all duration-200 w-full ${modalState.aiChat ? 'bg-primary text-white shadow-sm' : 'text-gray-600 hover:text-primary hover:bg-gray-50'}`}>
          <div className={`${modalState.aiChat ? 'text-white' : 'text-gray-400 group-hover:text-gray-600'} flex-shrink-0`}><Brain size={isCollapsed ? 22 : 20} /></div>
          <AnimatePresence>{!isCollapsed && (<motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="font-medium whitespace-nowrap overflow-hidden">AI Assistant</motion.span>)}</AnimatePresence>
        </button>
      </Restricted>

      {/* Hide Usage stats from Employees */}
      <Restricted to="not:employee">
      ...
      <Restricted to="canAccessGameSpace">
      ...
      <Restricted to="canViewNotifications">
      ...
      {/* Hide System Logs from Employees (Complex logic handled by Restricted or fallback to JS if Restricted is simple) */}
      {/* Assuming Restricted handles permission check "canViewSystemLog" OR Owner override internally */}
      <Restricted to="canViewSystemLog">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col transition-colors duration-300">
      ...
      <div className="lg:hidden bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40">
        <div className="flex items-center justify-between p-4">
          <div>
            ...
            <p className="text-sm text-gray-600 truncate">{getPageDescription()}</p>
          </div>
          <button onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)} className="p-2 rounded-lg text-gray-600 hover:text-primary hover:bg-gray-100 transition-colors">
      ...
      <div className="flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 h-full">
          ...
          <AnimatePresence>
            {isMobileSidebarOpen && (
              <>
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 lg:hidden" onClick={() => setIsMobileSidebarOpen(false)} />
                <motion.aside initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'tween', duration: 0.3 }} className="fixed top-0 left-0 h-full w-64 bg-white shadow-xl z-50 lg:hidden flex flex-col">
                  <div className="p-4 flex flex-col h-full">
                    <div className="flex items-center justify-between mb-6">
                      <h2 className="text-lg font-semibold text-primary">Navigation</h2>
                      <button onClick={() => setIsMobileSidebarOpen(false)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"><X size={20} /></button>
                    </div>
                    <nav className="space-y-2 flex-1 overflow-y-auto">{renderSidebarLinks(false)}</nav>
                  </div>
                </motion.aside>
              </>
            )}
          ...
          {/* Desktop Sidebar */}
          <div className={`hidden lg:block shrink-0 ${isSidebarCollapsed ? 'lg:w-25' : 'lg:w-64'} transition-all duration-300`}>
            <motion.aside style={{ height: `${sidebarHeight}px` }} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden sticky top-32 flex flex-col" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <div className="p-4 flex flex-col h-full">
                <div className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} mb-6`}>
                  {!isSidebarCollapsed && <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-lg font-semibold text-primary">Navigation</motion.h2>}
                  <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors hidden lg:flex">
                    {isSidebarCollapsed ? <ChevronRight size={16} /> : <ChevronLeft size={16} />}
                  </button>
                </div>
                <nav className="space-y-2 flex-1 overflow-y-auto">{renderSidebarLinks(isSidebarCollapsed)}</nav>
                <div className={`pt-4 border-t border-gray-200 mt-4 ${isSidebarCollapsed ? 'text-center' : ''}`}>
                  {!isSidebarCollapsed && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs text-gray-500">E Manager v1.0</motion.p>}
                </div>
              </div>
            </motion.aside>
          </div>
          ...
          <main className="flex-1 min-w-0 lg:mt-0 flex flex-col">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 hidden lg:block">
              <h1 className="text-2xl lg:text-3xl font-bold text-primary mb-2">{getPageTitle()}</h1>
              <p className="text-gray-600">{getPageDescription()}</p>
            </motion.div>
            <div className="px-4 sm:px-6 lg:px-8 pt-4"><AnnouncementBanner /></div>
            <motion.div key={location.pathname} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex-1 flex flex-col">
              <div className="p-4 lg:p-6 flex-1 overflow-y-auto"><Outlet /></div>
            </motion.div>
          </main>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="mb-6 md:mb-0 text-center md:text-left">
              ...
              <p className="text-gray-600 text-sm mt-2">The AI-Powered Team Command Center.</p>
            </div>
            <div className="flex items-center bg-gray-200 rounded-xl p-1">
              <button onClick={() => setTheme('light')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${theme === 'light' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900 hover:bg-white/50'}`}><Sun size={14} /><span>Light</span></button>
              <div className="w-px h-4 bg-gray-300 mx-1"></div>
              <button onClick={() => setTheme('dark')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${theme === 'dark' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900 hover:bg-white/50'}`}><Moon size={14} /><span>Dark</span></button>
              <div className="w-px h-4 bg-gray-300 mx-1"></div>
              <button onClick={() => setTheme('system')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${theme === 'system' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900 hover:bg-white/50'}`}><Laptop size={14} /><span>System</span></button>
            </div>
            <div className="flex flex-col justify-center gap-2 text-rose-600 text-sm">
              ...
              <div className="flex justify-center md:justify-end space-x-6">
                <Link to="/privacy" className="text-gray-600 hover:text-gray-800 transition-colors text-sm font-medium">Privacy Policy</Link>
                <Link to="/terms" className="text-gray-600 hover:text-gray-800 transition-colors text-sm font-medium">Terms of Service</Link>
                <Link to="/support" className="text-gray-600 hover:text-gray-800 transition-colors text-sm font-medium">Support</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Global Modals */}
      <UpgradeModal isOpen={modalState.upgradeModal} onClose={() => closeModal('upgradeModal')} message={modalContext?.message} />
      <CommandPalette />
      <ShortcutsModal />
      <AiChatModal />
      <OnboardingModal isOpen={showOnboardingModal} onClose={() => setShowOnboardingModal(false)} />

      {/* Create Employee Modal (Visible to Owner/Manager) */}
      <CreateEmployeeModal
        isOpen={modalState.createEmployee}
        onClose={() => closeModal('createEmployee')}
        teamId={modalContext?.teamId}
        onEmployeeCreated={() => {
          if(modalContext?.onEmployeeCreated) modalContext.onEmployeeCreated();
