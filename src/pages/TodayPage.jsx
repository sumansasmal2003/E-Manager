...
const QuickAddNote = ({ onNoteAdded }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-200 shadow-sm overflow-hidden"
    >
      {!isExpanded ? (
        <button
          onClick={toggle}
...
const StatsCard = ({ icon, title, value, color, description, onClick }) => (
  <motion.button
    ...
    onClick={onClick}
    className={`bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 text-center w-full transition-all duration-200 hover:shadow-md hover:scale-[1.02] active:scale-[0.98] ${
...
// Task Item Component
const TaskItem = ({ task, type, isOverdue = false }) => {
  const getTaskConfig = () => {
    if (isOverdue) {
      return {
    ...
    return {
      icon: <CalendarCheck size={16} className="text-amber-600" />,
      label: 'Due Today',
  ...
  return (
    <Link
      to={`/team/${task.team}`}
      className={`block p-4 rounded-lg border transition-all duration-200 group ${config.className}`}
    >
...
const TodayPage = () => {
  ...
  // Expandable states for action items
  ...
  // Fetch briefing with session caching
  useEffect(() => {
    const fetchBriefing = async () => {
      ...
      try {
        ...
        if (cachedData && cachedData.timestamp === today) {
          setBriefing(cachedData.briefing);
          setBriefingLoading(false);
          return;
        }
      } catch (e) {
        console.warn('Could not parse briefing cache', e);
        sessionStorage.removeItem('dailyBriefingData');
      }
      ...
      try {
        ...
        sessionStorage.setItem('dailyBriefingData', JSON.stringify(dataToCache));
      } catch (err) {
        console.error(err);
        setBriefingError('Failed to load AI briefing.');
      }
      setBriefingLoading(false);
    };
  ...
  const stats = useMemo(() => {
    ...
    const totalUrgentTasks = overdueTasks + dueTodayTasks;
  ...
  const navigationTabs = useMemo(() => [
    ...
    { id: SECTIONS.TASKS, name: 'Tasks', icon: CheckCircle },
    { id: SECTIONS.ROLL_CALL, name: 'Roll Call', icon: UserCheck },
    { id: SECTIONS.PLAN, name: 'Plan', icon: FileText }
  ], []);
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        ...
        >
          ...
          >
            ...
            {briefingError && !briefingLoading && (
              <div className="bg-yellow-50 rounded-2xl border border-yellow-200 p-6">
                <h3 className="text-yellow-800 font-semibold">Could not load AI Briefing</h3>
                <p className="text-yellow-700 text-sm">{briefingError}</p>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 my-6">
            ...
            <StatsCard
              ...
              onClick={() => {
                setActiveSection(SECTIONS.TASKS);
                overdueSection.expand();
              }}
            />
            <StatsCard
            ...
            <StatsCard
              ...
              onClick={() => setActiveSection(SECTIONS.PLAN)}
            />
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        {actionTasks.overdue.length === 0 && actionTasks.dueToday.length === 0 ? (
                          ...
                          <>
                            ...
                            {actionTasks.overdue.length > 0 && (
                              <ExpandableSection
                                title="Overdue Tasks"
                                count={actionTasks.overdue.length}
                                icon={<AlertTriangle size={18} className="text-red-600" />}
                                badgeColor="bg-red-100 text-red-700"
                                defaultExpanded={overdueSection.isExpanded}
                              >
                                <div className="space-y-3">
                            ...
                            {actionTasks.dueToday.length > 0 && (
                              <ExpandableSection
                                title="Due Today"
                                count={actionTasks.dueToday.length}
                                icon={<CalendarCheck size={18} className="text-amber-600" />}
                                badgeColor="bg-amber-100 text-amber-700"
                                defaultExpanded={dueTodaySection.isExpanded}
                              >
                                <div className="space-y-3">
          ...
          <div className="lg:col-span-1 space-y-4 sm:space-y-6">
            <AnimatePresence>
              {/* Roll Call Section */}
              {(activeSection === SECTIONS.ROLL_CALL || !window.matchMedia('(max-width: 1024px)').matches) && (
                <motion.div
                  key="rollcall"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
