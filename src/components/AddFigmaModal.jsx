...
// You'll need to get the Figma icon or another one. We'll use 'Link' for now.
// You can install react-icons (npm install react-icons) and use:
// import { FaFigma } from 'react-icons/fa';

const AddFigmaModal = ({ isOpen, onClose, teamId, onFigmaLinkAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const res = await api.post(`/teams/${teamId}/figma`, { name, link });
      onFigmaLinkAdded(res.data);
      setLoading(false);
      onClose();
      setName('');
      setLink('');
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Add Figma File">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Input
          ...
          type="text"
          placeholder="File name (e.g., 'Main UI Design')"
          value={name}
        ...
        <Input
          ...
          type="url"
          placeholder="Figma file link (https://...)"
          value={link}
